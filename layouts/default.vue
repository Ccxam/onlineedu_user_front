...
<!--            </li>-->
<!--            &lt;!&ndash; /未登录显示第1 li；登录后显示第2，3 li &ndash;&gt;-->
<!--          </ul>-->
          <ul class="h-r-login">
  ...
  <script>
      ...
      export default {
          ...
          methods:{
              ...
              logout(){
                  //清空cookie中的值
                  cookie.set('edu_token','',{domain:'localhost',expires:-1})
                  cookie.set('edu_userInfo','',{domain:'localhost',expires:-1})
                  //回到首页面
              ...
              wxLogin(){
                  ...
                  //调用接口根据token值获取userInfo
                  loginApi.getLoginInfo(this.token).then(res=>{
                      this.loginInfo = res.data.data.userInfo;
                      cookie.set('edu_userInfo',this.loginInfo,{domain:'localhost'})
                  })
              }
            },
      created() {
